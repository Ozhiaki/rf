...
pub fn limit(value: &str) -> Result<usize, String> {
    let parsed = value.parse::<usize>().map_err(|_| "limit must be a positive integer".to_string())?;
...
fn row_key(row: &Value) -> (&str, &str) {
    let attribution = row["stage"].as_str().or_else(|| row["surfaced_by"].as_str()).unwrap_or("");
...
pub fn page(mut all: Vec<Value>, query: &Value, limit: usize, cursor: Option<&str>) -> Result<Page, Error> {
    ...
    let start = match cursor {
        ...
        Some(cursor) => {
            ...
            if fields.len() != 4 || fields[0] != expected_version || fields[1] != query_hash
                || fields[2].len() != 64 || !fields[2].bytes().all(|byte| byte.is_ascii_hexdigit()) {
                return Err(Error::InvalidCursor);
            }
            let position = fields[3].parse::<usize>().map_err(|_| Error::InvalidCursor)?;
            if position > all.len() {
                return Err(Error::InvalidCursor);
            }
    ...
    let next_cursor = (end < all.len()).then(|| format!("v{CONTRACT_VERSION}.{query_hash}.{snapshot_hash}.{end}"));
    Ok(Page { data: all[start..end].to_vec(), total: all.len(), snapshot_hash, next_cursor })
}
